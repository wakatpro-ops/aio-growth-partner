...
create policy "read org search visibility settings" on public.search_visibility_settings
for select using (public.is_org_member(organization_id) or public.is_platform_admin());
create policy "write org search visibility settings" on public.search_visibility_settings
...
create policy "read org search visibility keywords" on public.search_visibility_keywords
for select using (public.is_org_member(organization_id) or public.is_platform_admin());
create policy "write org search visibility keywords" on public.search_visibility_keywords
for all using (public.is_org_member(organization_id) or public.is_platform_admin())
with check (public.is_org_member(organization_id) or public.is_platform_admin());

create policy "read org search visibility snapshots" on public.search_visibility_snapshots
for select using (public.is_org_member(organization_id) or public.is_platform_admin());
create policy "write org search visibility snapshots" on public.search_visibility_snapshots
for all using (public.is_org_member(organization_id) or public.is_platform_admin())
with check (public.is_org_member(organization_id) or public.is_platform_admin());
