...
export default async function NewCustomerPage({ params }: { params: Promise<{ storeId: string }> }) {
  ...
  return (
    <AppShell>
      ...
      <CustomerForm action={createCustomerAction.bind(null, store.id)} showVehicle={store.industry_type_key === "auto_repair"} cancelHref={`/stores/${store.id}/customers?tab=customers`} />
    </AppShell>
  );
}
