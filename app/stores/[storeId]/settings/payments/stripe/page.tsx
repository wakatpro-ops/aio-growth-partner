    ...
    <AppShell>
      ...
      <section className="grid cols-2">
        <article className="card">
          ...
          <p>店舗が自分のStripeアカウントで決済を受け、AIOでは請求書・入金状況と連動して管理します。</p>
          ...
          <div className="actions-row">
            {connectEnvReady ? (
              <Link className="button" href={`/api/stripe/oauth/start?storeId=${store.id}`}>
                {isConnected ? "別のStripeアカウントに接続" : "Stripeに接続"}
              </Link>
            ) : (
      ...
      <section className="card form">
        <h2>手動連携情報</h2>
        <p>Stripe Connectを使わない場合や、移行期間中は手動でアカウント情報を管理できます。</p>
