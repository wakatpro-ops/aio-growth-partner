...
const registerLabels: Record<string, string> = { external_pos: "既存のPOS・レジを使う", simple_register: "AIO boostの簡易会計を使う", not_needed: "会計・レジは不要", undecided: "後で決める" };
...
export function InitialSetupReviewForm({ review }: { review: InitialSetupReview }) {
  ...
  const [registerMode, setRegisterMode] = useState(draftOperatingModel.register.mode);
  ...
  const answered = [structureLabels[structureMode], authorityLabels[systemAuthorities.reservations], registerLabels[registerMode], `${store.name}の店舗情報`, `${selectedMenus.length}件のメニュー`, `${invoice.issuerName}の請求書設定`];
      ...
      {currentStep > 0 ? (
        <section className="setup-chat card">
          ...
          <div className="setup-chat-thread">
            ...
            {currentStep === 5 ? <MenuQuestion menus={menus} showEditor={showMenuEditor} onToggleEditor={() => setShowMenuEditor((visible) => !visible)} onMenuChange={setMenu} onAcceptAll={() => { setMenus((items) => items.map((menu) => ({ ...menu, enabled: true }))); goTo(6); }} onNext={() => goTo(6)} /> : null}
            {currentStep === 6 ? <InvoiceQuestion value={invoice} onChange={setInvoice} onNext={() => goTo(7)} /> : null}
            {currentStep === 7 ? <FinalSummary structureMode={structureMode} systemAuthorities={systemAuthorities} setSystemAuthorities={setSystemAuthorities} registerMode={registerMode} serviceMode={serviceMode} setServiceMode={setServiceMode} sharing={sharing} setSharing={setSharing} locations={locations} setLocation={setLocation} store={store} selectedMenuCount={selectedMenus.length} featureCount={selectedPreset.recommendedFeatures.length} skippedCount={skipped.length} confirmed={finalConfirmed} setConfirmed={setFinalConfirmed} /> : null}
          </div>
          <footer className="setup-chat-actions">{currentStep > 1 ? <button className="button secondary" type="button" onClick={() => goTo(currentStep - 1)}>一つ前に戻る</button> : <button className="button secondary" type="button" onClick={() => goTo(0)}>準備内容へ戻る</button>}{currentStep <= questionCount ? <button className="button secondary" type="button" onClick={() => skip(["structure", "reservation", "register", "store", "menus", "invoice"][currentStep - 1] ?? `step-${currentStep}`, currentStep === 5 ? () => setMenus((items) => items.map((menu) => ({ ...menu, enabled: false }))) : undefined)}>後で確認する</button> : null}<PendingSubmitButton className="button ghost" formAction={saveDraftAction} pendingLabel="途中保存しています...">途中保存して終了</PendingSubmitButton></footer>
        </section>
      ) : null}
    </form>
  );
...
function RegisterQuestion({ mode, onChoose }: { mode: InitialSetupReview["operatingModel"]["register"]["mode"]; onChoose: (value: InitialSetupReview["operatingModel"]["register"]["mode"]) => void }) {
  return <><div className="setup-bubble ai"><p>会計やレジについて教えてください。すでに利用中のレジがあれば、その仕組みを残して連携できます。</p><small>AIO boostの簡易会計は、レジを持っていない店舗だけ利用できます。</small></div><div className="setup-choice-grid">{Object.entries(registerLabels).map(([value, label], index) => <button key={value} className={`setup-choice ${mode === value ? "selected" : ""}`} type="button" onClick={() => onChoose(value as InitialSetupReview["operatingModel"]["register"]["mode"])}><span>{index + 1}</span>{label}</button>)}</div></>;
}
