...
export default async function AdminOrganizationsPage({ searchParams }: { searchParams: Promise<{ view?: string; archived?: string; restored?: string }> }) {
  ...
  return (
    <AppShell>
      <PageHeader title="組織管理" description="契約組織、所属ユーザー、店舗数を確認し、利用終了した組織をアーカイブできます。" />
      {query.archived ? <p className="notice success">組織をアーカイブしました。</p> : null}
      {query.restored ? <p className="notice success">組織を復元しました。</p> : null}
      <div className="button-row"><Link className={`button ${showArchived ? "secondary" : ""}`} href="/admin/organizations">利用中</Link><Link className={`button ${showArchived ? "" : "secondary"}`} href="/admin/organizations?view=archived">アーカイブ済み</Link></div>
        ...
        <table className="table">
          ...
          <tbody>
            {organizations.map((organization) => <tr key={organization.id}>
              ...
              <td>{showArchived ? <form action={restoreAdminOrganizationAction.bind(null, organization.id)}><button className="button secondary" type="submit">復元</button></form> : <form action={archiveAdminOrganizationAction.bind(null, organization.id)}><ConfirmSubmitButton message={`「${organization.name}」をアーカイブします。所属する利用中店舗がある場合は先に店舗をアーカイブしてください。`}>アーカイブ</ConfirmSubmitButton></form>}</td>
            </tr>)}
            {organizations.length === 0 ? <tr><td colSpan={5}>{showArchived ? "アーカイブ済み組織はありません。" : "利用中組織はありません。"}</td></tr> : null}
          </tbody>
        </table>
      </section>
    </AppShell>
