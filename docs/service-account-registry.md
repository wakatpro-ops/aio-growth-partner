...
| LINE Official Account / Messaging API | LINEログイン `若園 忠義 waka` / organization `AIOBoostサポート` (`BM74147211322`) | account `AIOBoostサポート` / Basic ID `@056miwxi` / Channel ID `2011470356` | 専用アカウントなし | 2026-09-06にMessaging APIを有効化し、provider `株式会社 Navi Life`、Privacy `https://app.aioboost.jp/privacy`、Terms `https://app.aioboost.jp/terms` を登録。Channel secretは発行済みだが台帳へ記録しない。Webhook URL・Channel Access Token・Vercel本番環境変数は未設定。管理画面のプラン表示は「コミュニケーション」だが、課金状態は未確認（UNKNOWN） |
