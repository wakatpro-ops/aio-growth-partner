...
| SendGrid | account `My SendGrid Account` | authenticated domain `aioboost.jp` / inbound host `in.aioboost.jp` | 本番と同じWebhook秘密値をVercel stagingへ設定（Inbound Parseの送信先は本番のみ） | 2026-09-07確認。画面表示はTrial。Value DomainへDomain Authentication用CNAMEとInbound Parse用MXを設定し、SendGridでドメイン検証済み。Inbound Parseは本番の`/api/inbound/store-email`へ接続。Webhook秘密値はVercel Production・stagingでローテーションし、値は記録しない。ルートMXのGoogle Workspaceは維持。実メールが対象店舗だけへ到着し、問い合わせ分類・要約・要確認保存まで成功 |
...
- 2026-09-07: Issue #144の店舗固有AI受信箱を本番稼働。SendGrid Trialで`aioboost.jp`を認証し、`in.aioboost.jp`のMXとInbound Parseを設定。Vercel Production・stagingへ専用Webhook秘密値を設定して再デプロイし、秘密値なしPOSTが両環境でHTTP 401になることを確認した。店舗「nana」の固有アドレスへGmailから送った実メールが同店舗のみに到着し、「問い合わせ」として要約・要確認保存されるエンドツーエンド試験に成功。ルート`aioboost.jp`のGoogle Workspace MXは変更していない。秘密値・店舗固有アドレスは台帳へ記録しない。
- 2026-09-08: Issue #148として、店舗AI受信箱に予約メール形式の初回承認・店舗別学習・同一形式の自動処理を追加。新規・変更・キャンセルを別形式として記憶し、形式変更、項目不足、重複、日時衝突、対象予約なしは確認待ちへ戻す。未所属、別法人・別店舗、閲覧者、停止アカウント、スタッフによる学習、DB直接操作の負の統合テストを含む8件をステージングで通過。既存Vercel Pro・Supabase Proを再利用し、新しい有料サービスは追加しない。ステージングVercelはSupabaseの新形式publishable/secretキーへ移行し、再デプロイ後に受信API、未認証拒否、ログイン画面の疎通を確認。秘密値は台帳・Gitへ記録しない。
