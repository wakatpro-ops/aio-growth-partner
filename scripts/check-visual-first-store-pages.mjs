...
assert.match(inventory, /残量を確認/u);
assert.match(inventory, /stock.quantity.*stock.reserved_quantity/u);
assert.ok(inventory.indexOf('className="menu-card-grid"') < inventory.indexOf('在庫の変動履歴'));
assert.match(customers, /来店状況/u);
assert.match(customers, /連絡可否/u);
assert.match(customers, /tab === "analysis"/u);
assert.match(customers, /BookingCalendar/u);
assert.match(customers, /DataPreview/u);
assert.match(items, /inventory\?tab=menu/u);
assert.match(read("components/menu/menu-cards.tsx"), /ItemThumbnail/u);
assert.match(read("components/menu/menu-cards.tsx"), /availabilityLabels/u);
