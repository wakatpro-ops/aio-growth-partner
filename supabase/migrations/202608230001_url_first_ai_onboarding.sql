...
create index if not exists public_store_analyses_created_idx
  on public.public_store_analyses(created_at desc);
create index if not exists public_store_analyses_rate_limit_idx
  on public.public_store_analyses(rate_limit_key, created_at desc)
  where rate_limit_key is not null;
create index if not exists public_store_analyses_expiry_idx
  on public.public_store_analyses(expires_at)
  where converted_application_id is null;

alter table public.public_store_analyses enable row level security;

-- Intentionally no anon/authenticated policies. Public access is only through
-- bounded server routes using the service role; direct listing is prohibited.
...
-- Applications are accepted only by the validated server route. Anonymous
-- callers must not bypass URL-token validation with a direct PostgREST insert.
