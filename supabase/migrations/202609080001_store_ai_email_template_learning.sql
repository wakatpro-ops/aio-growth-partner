-- Issue #148: learn a store-scoped booking email structure after one explicit approval.
-- Templates contain only a structural hash and routing metadata, never raw mail or customer values.

create table if not exists public.store_ai_email_templates (
  ...
  event_type text not null check (event_type in ('created','changed','cancelled')),
  template_fingerprint text not null,
  status text not null default 'active' check (status in ('active','paused')),
  approved_message_id uuid references public.store_ai_email_messages(id) on delete set null,
  match_count integer not null default 1 check (match_count >= 0),
  auto_processed_count integer not null default 0 check (auto_processed_count >= 0),
  ...
  constraint store_ai_email_templates_store_org_fkey foreign key (store_id, organization_id)
    references public.stores(id, organization_id) on delete cascade,
  constraint store_ai_email_templates_sender_check check (sender_email = lower(sender_email) and sender_email ~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'),
  constraint store_ai_email_templates_provider_check check (provider_key ~ '^[a-z0-9_]{2,80}$'),
  constraint store_ai_email_templates_fingerprint_check check (template_fingerprint ~ '^[a-f0-9]{64}$')
);

create unique index if not exists store_ai_email_templates_active_uidx
  on public.store_ai_email_templates(store_id, lower(sender_email), provider_key, event_type, template_fingerprint)
  where archived_at is null;
create index if not exists store_ai_email_templates_store_status_idx
  on public.store_ai_email_templates(store_id, status, updated_at desc) where archived_at is null;

alter table public.store_ai_email_templates enable row level security;
revoke all on public.store_ai_email_templates from public, anon, authenticated;
grant all on public.store_ai_email_templates to service_role;

alter table public.store_ai_email_messages
  add column if not exists booking_event_type text check (booking_event_type in ('created','changed','cancelled')),
  add column if not exists booking_provider text,
  add column if not exists template_fingerprint text,
  add column if not exists matched_template_id uuid references public.store_ai_email_templates(id) on delete set null;

alter table public.store_ai_email_messages
  drop constraint if exists store_ai_email_messages_template_fingerprint_check;
alter table public.store_ai_email_messages
  add constraint store_ai_email_messages_template_fingerprint_check
  check (template_fingerprint is null or template_fingerprint ~ '^[a-f0-9]{64}$');
...
-- Keep the former RPC as a compatibility wrapper for old server code and tests.
...
set search_path = public
as $$
  select public.apply_store_ai_email_event(p_message_id, p_actor_user_id, p_automatic, false);
$$;
